...
abstract class AbstractRecipe implements Recipe {
    ...
    protected Double remainingSecondsUntilDone = -60.0;   //set a non-0 default value to avoid the wrong response of isRecipeDone().

    protected int ovenTemperature = 0;
    ...
    @Override public void adjust(Time unit, int amount, int ovenTemperature) {     //Because different temperature needs different time, so it's possible the remaining time is larger than before after adjust().
        ...
        if (remainingSecondsUntilDone - amount < 0) {
            ...
            remainingSecondsUntilDone = (remainingSecondsUntilDone - amount) / this.ovenTemperature * ovenTemperature;   //Here is the reason of exist of larger remaining time after adjust().
